    ...
    <div className="p-8 max-w-5xl">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-stone-500 mt-1">Create tools your agents can use during calls</p>
          </div>
          <button
        ...
        {loading ? (
          <div className="space-y-4">
            {[1, 2].map((i) => (
              <div key={i} className="bg-white rounded-xl border border-stone-200 p-6 animate-pulse bg-stone-100">
          ...
          <div className="space-y-4">
            {tools.map((tool) => (
              <div key={tool.id} className="bg-white rounded-xl border border-stone-200 p-6 hover:shadow-[0_0_30px_rgba(0,180,216,0.08)] transition-shadow">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-stone-100 text-stone-500">{tool.type}</span>
                      </div>
                    </div>
                    <p className="text-sm text-stone-500 mt-2">{tool.description}</p>
                    ...
                    {tool.agents.length > 0 && (
                      <div className="mt-3 flex items-center gap-2">
                        <span className="text-xs text-stone-500">Used by:</span>
                        <div className="flex gap-1">
                          {tool.agents.map((agent) => (
                            <span key={agent.id} className="text-xs bg-amber-50 text-amber-600 px-2 py-0.5 rounded-full">{agent.name}</span>
                          ))}
                        </div>
                      </div>
                    )}
          ...
          <div className="fixed inset-0 bg-stone-900/50 flex items-center justify-center z-50 p-4" onClick={() => setDeleteConfirm(null)}>
            <div className="bg-white rounded-2xl w-full max-w-sm p-6" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex gap-3">
                <button onClick={() => setDeleteConfirm(null)} className="flex-1 py-2.5 border border-stone-200 rounded-lg font-medium text-stone-700 hover:bg-white">Cancel</button>
                <button onClick={() => deleteTool(deleteConfirm)} className="flex-1 py-2.5 bg-red-600 text-stone-900 rounded-lg font-medium hover:bg-red-700">Delete</button>
              </div>
            </div>
          </div>
        )}
    ...
    <div className="fixed inset-0 bg-stone-900/50 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="sticky top-0 bg-white p-6 border-b border-stone-200 flex items-center justify-between z-10">
          ...
          <button onClick={onClose} className="p-2 text-stone-500 hover:text-stone-500 rounded-lg"><X className="w-5 h-5" /></button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          {/* Tool Type Selection */}
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-2">Tool Type</label>
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                ...
                className={`p-4 border rounded-xl text-left transition-colors ${type === 'transfer' ? 'border-brand-500 bg-amber-50/50' : 'border-stone-200 hover:border-stone-200'}`}
              >
                <Phone className="w-5 h-5 text-green-500 mb-2" />
                <div className="font-medium text-sm text-stone-900">Transfer Call</div>
                <div className="text-xs text-stone-500">Transfer to another number</div>
              </button>
            </div>
          </div>
          ...
          {/* Basic Info */}
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1.5">Tool Name</label>
            <input
              type="text"
              value={form.name}
          ...
          {type === 'api' && (
            <div className="space-y-4 bg-white rounded-xl p-4">
              <div className="flex items-center gap-2 text-sm font-medium text-stone-700"><Globe className="w-4 h-4" /> API Configuration</div>
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="col-span-2">
                  <label className="block text-xs text-stone-500 mb-1">Endpoint URL</label>
                  <input
                    type="url"
              ...
              <div>
                <label className="block text-xs text-stone-500 mb-1">Headers (JSON)</label>
                <textarea
          ...
          {type === 'transfer' && (
            <div className="space-y-4 bg-white rounded-xl p-4">
              <div className="flex items-center gap-2 text-sm font-medium text-stone-700"><Phone className="w-4 h-4" /> Transfer Configuration</div>
              <div>
                <label className="block text-xs text-stone-500 mb-1">Transfer Number</label>
                <input
                  type="tel"
              ...
              <div>
                <label className="block text-xs text-stone-500 mb-1">Transfer Message</label>
                <input
                  type="text"
          ...
          <div className="bg-white rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2 text-sm font-medium text-stone-700"><Code className="w-4 h-4" /> Parameters Schema (JSON)</div>
            <textarea
              ...
              className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm font-mono"
            />
            <p className="text-xs text-stone-500">Define the parameters the AI should collect before calling this tool.</p>
          </div>
    ...
    <div className="fixed inset-0 bg-stone-900/50 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1.5">Tool Name</label>
            <input type="text" value={form.name} onChange={(e) => update('name', e.target.value)} className="w-full px-4 py-2.5 border border-stone-200 rounded-lg focus:ring-2 focus:ring-cyan-500 outline-none" required />
          </div>

          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1.5">Description</label>
            <textarea value={form.description} onChange={(e) => update('description', e.target.value)} rows={3} className="w-full px-4 py-2.5 border border-stone-200 rounded-lg focus:ring-2 focus:ring-cyan-500 outline-none resize-none" required />
          </div>
          ...
          {tool.type === 'api' && (
            <div className="space-y-4 bg-white rounded-xl p-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-1">
                  <label className="block text-xs text-stone-500 mb-1">Method</label>
                  <select value={form.apiMethod} onChange={(e) => update('apiMethod', e.target.value)} className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm">
                    <option>POST</option><option>GET</option><option>PUT</option><option>DELETE</option>
                  </select>
                </div>
                <div className="col-span-2">
                  <label className="block text-xs text-stone-500 mb-1">Endpoint URL</label>
                  <input type="url" value={form.apiEndpoint} onChange={(e) => update('apiEndpoint', e.target.value)} className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm" />
                </div>
              </div>
              <div>
                <label className="block text-xs text-stone-500 mb-1">Headers (JSON)</label>
                <textarea value={form.apiHeaders} onChange={(e) => update('apiHeaders', e.target.value)} rows={3} className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm font-mono" />
              </div>
            </div>
          )}
          ...
          {tool.type === 'transfer' && (
            <div className="space-y-4 bg-white rounded-xl p-4">
              ...
              <div>
                <label className="block text-xs text-stone-500 mb-1">Transfer Number</label>
                <input type="tel" value={form.transferNumber} onChange={(e) => update('transferNumber', e.target.value)} className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm" />
              </div>
              <div>
                <label className="block text-xs text-stone-500 mb-1">Transfer Message</label>
                <input type="text" value={form.transferMessage} onChange={(e) => update('transferMessage', e.target.value)} className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm" />
              </div>
            </div>
          )}
          ...
          <div className="bg-white rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2 text-sm font-medium text-stone-700"><Code className="w-4 h-4" /> Parameters Schema (JSON)</div>
            <textarea value={form.parameters} onChange={(e) => update('parameters', e.target.value)} rows={6} className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm font-mono" />
          </div>
